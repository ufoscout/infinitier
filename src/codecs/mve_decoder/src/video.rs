...
fn decode8_0x2(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
...
fn decode8_0x3(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let b = r.read_byte()? as i32;
    let (x, y) = if b < 56 {
        (-(8 + b % 7), -(b / 7))
    } else {
...
fn decode8_0x7(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let p0 = r.read_byte()?;
    let p1 = r.read_byte()?;
...
fn decode8_0x8(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let p: [u8; 8];
    let b: [u8; 8];
    ...
    if p0 <= p1 {
        ...
        for y in 0..8usize {
            ...
            for x in 0..8usize {
                ...
                buf[dst + y * width + x] = if flags & (1 << (y % 4 * 8 + x)) != 0 {
                    pp1
                } else {
                    pp0
                };
        ...
        if p2 <= p3 {
            ...
            for y in 0..8usize {
                let flags =
                    pack_flags_8(&b, y / 4 * 2, y / 4 * 2 + 1, y / 4 * 2 + 4, y / 4 * 2 + 5);
                let mut pp0 = p[0];
                let mut pp1 = p[1];
                for x in 0..8usize {
                    if x == 4 {
...
fn decode8_0x9(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let p: [u8; 4] = [
        r.read_byte()?,
        r.read_byte()?,
        r.read_byte()?,
...
fn decode8_0xa(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let p0 = r.read_byte()?;
    let p1 = r.read_byte()?;
    let p2 = r.read_byte()?;
    let p3 = r.read_byte()?;
...
fn decode8_0xc(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let mut y = 0;
    while y < 8 {
        let mut x = 0;
        while x < 8 {
...
fn decode8_0xd(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
    let p: [u8; 4] = [
        r.read_byte()?,
        r.read_byte()?,
        r.read_byte()?,
...
fn decode8_0xf(r: &mut Reader8<'_>, buf: &mut [u8], dst: usize, width: usize) -> Result<(), Error> {
...
impl<'a> Reader16<'a> {
    ...
    fn read_u16(&mut self) -> Result<u16, Error> {
        if self.pos + 2 > self.data.len() {
            return Err(Error::VideoDecode(
                "unexpected end of 16-bit video data (u16)".into(),
            ));
        }
        ...
        for y in 0..8usize {
            ...
            for x in 0..8usize {
                write_px(
                    buf,
                    dst_px + y * width + x,
                    p[((flags >> (x * 2)) & 0x03) as usize],
                );
            }
        }
    } else if p[0] & 0x8000 == 0 {
        let p2 = p[2] & !0x8000;
        let p = [p[0], p[1], p2, p[3]];
        // per-2×2-block
        let flags = r.read_byte()? as u32
            | ((r.read_byte()? as u32) << 8)
            | ((r.read_byte()? as u32) << 16)
            ...
            for y in 0..8usize {
                ...
                for x in 0..8usize {
                    ...
                    write_px(
                        buf,
                        dst_px + y * width + x,
                        p[split + ((flags >> (x * 2)) & 0x03) as usize],
                    );
                }
            }
        } else {
...
pub fn decode_frame16(
    ...
    if offset > data.len() {
        return Err(Error::VideoDecode(
            "16-bit frame: invalid data2 offset".into(),
        ));
    }
