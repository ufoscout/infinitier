...
fn decode_and_hash(acm_rel: &str) -> (String, AcmInfo) {
    let acm_path = get_assets_path().join("ACM/ACM").join(acm_rel);
...
fn test_reset_after_full_decode_matches_fresh_decode() {
    let acm_path = get_assets_path().join("ACM/ACM").join("bg/Bf1d1.ACM");
    let data = DataSource::new(acm_path);
...
fn test_reset_preserves_output_channels_override() {
    ...
    // survives a reset.
    let acm_path = get_assets_path().join("ACM/ACM").join("bg/Bf1j4.ACM");
    let data = DataSource::new(acm_path);
