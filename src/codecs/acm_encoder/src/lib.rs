...
/// plus the source spec. Used by every `encode_wav*` entry point so the
/// header-validation plus sample-collection boilerplate lives in one place.
...
fn write_wavc<W: Write>(
    ...
    let uncompressed =
        u32::try_from(uncompressed).map_err(|_| AcmEncodeError::TooManySamples(samples_len))?;
