...
impl BamV2 {
    ...
    pub fn frame_to_image(
        ...
        for block in data_blocks {
            let pvrz_path = fs
                .search_path_opt(&block.pvrz_name())
...
mod tests {
    ...
    fn test_parse_bam_v2_02() {
        ...
        {
            let fs = CaseInsensitiveFS::new(get_assets_path().join("BAM/V2")).unwrap();
            let image = bam.frame_to_image(0, &fs).unwrap();

            assert_images_are_equal(
                &image::open(get_assets_path().join("BAM/V2/1CHELM0300000.PNG")).unwrap(),
                &image.into(),
                None,
            );
        }
