...
// Round-trip coverage for BCS / BAF / BCS now lives in
// `crate::baf_compile::roundtrip_tests::roundtrip_assets`, which iterates
// the same `assets/BCS/source/` corpus this module used to.
//
// The old `corpus_tests` here did a strict byte-compare between our
// decompiler output and NearInfinity's reference `.BAF` files. We kept
// that test in lockstep with full game extracts via the
// `EXTRACTED_RESOURCES` env var, but the NI references include
// string-table / RES-name annotations (e.g. `// Trap Sprung`,
// `// No such index`) that our decompiler intentionally does not emit,
// so the comparison is fragile against features we have no plans to
// re-implement. The round-trip test catches the cases this module
// actually needed to guard against (decompile + recompile equivalence),
// so the comparison test is redundant.
