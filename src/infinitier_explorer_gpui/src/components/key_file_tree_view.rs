...
//!
//! Expansion state, the current resource selection and the keyboard
//! cursor (`focused_row`) all live on `AppState` so a repaint keeps
...
use gpui::{
    Context, FocusHandle, FontWeight, InteractiveElement, IntoElement, ListSizingBehavior,
    ParentElement, ScrollStrategy, StatefulInteractiveElement, Styled, UniformListScrollHandle,
...
/// Key context the tree wrapper publishes. Key bindings in
/// `main.rs` target this context, so the same `SelectUp` etc.
/// shortcuts don't fight with any other tree later.
pub const KEY_FILE_TREE_CONTEXT: &str = "KeyFileTree";

// Tree-local arrow-key actions. `gpui-component` keeps its own
// `SelectUp`/etc. behind a `pub(crate)` module, so we declare our own
// unit-struct actions here and bind them in `main.rs`.
actions!(key_file_tree, [SelectUp, SelectDown, SelectLeft, SelectRight]);
...
/// in `AppState::focused_row` so the keyboard cursor survives a
/// collapse / expand. `TreeRow` has the same shape today but is an
/// internal layout type; we expose this separately so the state
/// module doesn't need a private import path.
...
impl KeyFileTreeView {
    ...
    fn build_rows(&self, expanded: &HashSet<&'static str>) -> Vec<TreeRow> {
        ...
        for (group_ix, group) in self.groups.iter().enumerate() {
            ...
            if expanded.contains(group.ext) {
    ...
    fn focused_row_index(&self, focused: FocusedRow, expanded: &HashSet<&'static str>) -> Option<usize> {
        // Headers come at predictable offsets — walk groups in order
        // and accumulate (1 header + expanded? leaf_count) per group.
        let mut acc = 0usize;
        for (gi, group) in self.groups.iter().enumerate() {
            let is_open = expanded.contains(group.ext);
            match focused {
...
// ── Action handlers ─────────────────────────────────────────────────
//
// All four are free functions taking `&mut ExplorerApp` because they
// touch state across `AppState` (focus, expansion, selection) and the
// `KeyFileTreeView` (scroll handle). Wiring is done in `render` via
// `cx.listener`.

/// Move the cursor up one row (no wrap). Auto-selects when the new
/// row is a leaf.
fn on_select_up(this: &mut ExplorerApp) {
    let rows = this.tree_view.build_rows(&this.state.expanded_groups);
    if rows.is_empty() {
        return;
    }
...
fn on_select_left(this: &mut ExplorerApp) {
    let Some(focused) = this.state.focused_row else {
        return;
    };
    match focused {
        FocusedRow::Header { group_ix } => {
            let ext = this.tree_view.groups[group_ix].ext;
            this.state.expanded_groups.remove(ext);
        }
        FocusedRow::Leaf { group_ix, .. } => {
            this.state.focused_row = Some(FocusedRow::Header { group_ix });
...
fn on_select_right(this: &mut ExplorerApp) {
    ...
    if let FocusedRow::Header { group_ix } = focused {
        let ext = this.tree_view.groups[group_ix].ext;
        this.state.expanded_groups.insert(ext);
    }
}
...
fn current_row_index(this: &ExplorerApp, rows: &[TreeRow]) -> Option<usize> {
    let focused = this.state.focused_row?;
    if let Some(ix) = this.tree_view.focused_row_index(focused, &this.state.expanded_groups) {
        return Some(ix);
    }
...
fn apply_cursor(this: &mut ExplorerApp, rows: &[TreeRow], new_ix: usize) {
    ...
    this.state.focused_row = Some(focused);
...
fn scroll_to_focus(this: &mut ExplorerApp) {
    ...
    if let Some(ix) = this
        .tree_view
        .focused_row_index(focused, &this.state.expanded_groups)
    {
        this.tree_view
            .scroll_handle
...
pub fn render(this: &mut ExplorerApp, cx: &mut Context<ExplorerApp>) -> impl IntoElement {
    let rows: Rc<Vec<TreeRow>> = Rc::new(this.tree_view.build_rows(&this.state.expanded_groups));
    let count = rows.len();
    ...
    let list = uniform_list(
        ...
        cx.processor(move |this, range: Range<usize>, _window, cx| {
            ...
            for row_ix in range {
                ...
                let focused = this.state.focused_row == Some(FocusedRow::from_row(row));
                let el = match row {
                    TreeRow::Header { group_ix } => {
                        let group = &this.tree_view.groups[group_ix];
                        let ext = group.ext;
                        let leaf_count = group.entries.len();
                        let is_open = this.state.expanded_groups.contains(ext);
                        ...
                        let mut row = h_flex()
                            ...
                            .on_click(cx.listener(move |this, _, _, cx| {
                                this.state.focused_row = Some(FocusedRow::Header { group_ix });
                                if !this.state.expanded_groups.remove(ext) {
                                    this.state.expanded_groups.insert(ext);
                                }
                                cx.notify();
                            }))
